...
for i in range(numTrackers):
    ...
    if(len(contours)>0):
        ...
        cv.imshow("test", mask)
        cv.waitKey(0)
        arrPoints.append(np.array([x -5 for x in center])) 
        arrPoints.append(np.array([x +5 for x in center])) 
    else:
...
while(1):
    ...
    if ret == True:
        ...
        frame = cv.line(frame, hand[0].center, hand[1].center, color=(150, 150, 150), thickness=2) 
        midpoint = ((np.array(hand[0].center) + np.array(hand[1].center))/2).astype(int)
